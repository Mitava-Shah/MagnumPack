      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div
            ...
            onClick={() => navigate(`/ProductCategory/editProductCategory/${row.original.slug}`)}
          >
            {row.original.photo && (
              <img
        ...
        Cell: ({ row }) => (
          <div className="flex gap-4">
            <button className="text-blue-500 hover:text-blue-700 transition">
              <Link to={`/ProductCategory/editProductCategory/${row.original.slug}`}>
                <FaEdit />
              </Link>
            </button>
            <button
...
/api/product/getall`, { withCredentials: true });
  ...
  const deleteCategory = async ({ id }) => {
    const url = `
/api/product/deletecategory?id=${id}`;
    try {
    ...
    <div className="p-4 overflow-x-auto">
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300">
          <Link to="/ProductCategory/CreateProductCategory">
            <FaPlus size={15} />
          </Link>
        </button>
      </div>
