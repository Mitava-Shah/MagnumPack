...
const WhyChoosePaperTable = () => {
  const [heading, setHeading] = useState("");
  const [subheading, setSubheading] = useState("");
  const [items, setItems] = useState([]); // Renamed from achievements
  const [loadings, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedWhyChoosePaper, setSelectedWhyChoosePaper] = useState(null); // State for the selected banner
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <span
            ...
            onClick={() => navigate(`/whychoosepaper/editwhychoosepaper/${row.original._id}`)}
          >
            {row.original.title}
          </span>
        ),
...

      {
        ...
        Cell: ({ value }) => {
          ...
          return firstImage ? <img src={`
/api/image/download/${firstImage}`} alt="Item" className="w-32 h-20 object-cover" /> : null;
        },
        disableSortBy: true,
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="flex gap-4">
            ...
            <button className="text-blue-500 hover:text-blue-700 transition">
              <Link to={`/whychoosepaper/editwhychoosepaper/${row.original._id}`}><FaEdit /></Link>
            </button>
  ...
  const handleView = (whyChoosepaper) => {
    setSelectedWhyChoosePaper(whyChoosepaper);
    setIsModalOpen(true);
  };

  const closeModal = () => {
    setIsModalOpen(false);
    setSelectedWhyChoosePaper(null);
  };
...
/api/whyChoosePaper/getAllWhyChoosePaper`, { withCredentials: true });
  ...
  const deleteItem = async (id) => {
    try {
      await axios.delete(`
/api/WhyChoosePaper/deleteWhyChoosePaper?id=${id}`, { withCredentials: true });
      fetchData();
    } catch (error) {
      console.error(error);
    }
  ...
  const saveHeadings = async () => {
    try {
      await axios.put('/api/pageHeading/updateHeading?pageType=whychoosepaper', {
        pagetype: 'whychoosepaper',
        heading,
        subheading,
  ...
  return (
    <div className="p-4 overflow-x-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-xl font-bold text-gray-700 font-serif uppercase">Why Choose Paper</h1>
        <button className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300 font-serif">
          <Link to="/whychoosepaper/createwhychoosepaper"><FaPlus size={15} /></Link>
        </button>
      </div>
      <div className="mb-4">
        <input
...
export default WhyChoosePaperTable;
