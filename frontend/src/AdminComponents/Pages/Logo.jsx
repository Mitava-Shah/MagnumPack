...
const LogoCRUD = () => {
  ...
  const handleTitleChange = (e, type) => {
    setImgTitles(prevState => ({ ...prevState, [type]: e.target.value }));
  };
  ...
  const handleUpload = async (type) => {
    try {
      ...
      formData.append('imgTitle', imgTitles[type]);
      ...
      setImgTitles(prevState => ({ ...prevState, [type]: '' }));
      ...
      {logoTypes.map((logoType) => (
        <div key={logoType.type} className="mb-6 flex flex-col md:flex-row gap-2 bg-gray-200 justify-around rounded-md">
          <div>
            ...
            <div className="mb-4">
              <label htmlFor={`title-${logoType.type}`} className="block font-semibold mb-2">
                Image Title
              </label>
              <input
                type="text"
                id={`title-${logoType.type}`}
                value={imgTitles[logoType.type] || ''}
                onChange={(e) => handleTitleChange(e, logoType.type)}
                className="p-2 border rounded focus:outline-none"
                required
              />
            </div>
          ...
          <div className="max-w-96">
            {logos[logoType.type] && (
              <div key={logos[logoType.type]._id} className="p-4 mt-6">
                ...
                <div className="mt-4">
                  <label htmlFor={`title-${logos[logoType.type]._id}`} className="block font-semibold mb-2">
                    Image Title
                  </label>
                  <input
                    type="text"
                    id={`title-${logos[logoType.type]._id}`}
                    value={logos[logoType.type].title || ''}
                    readOnly
                    className="p-2 border rounded focus:outline-none"
                  />
                </div>
