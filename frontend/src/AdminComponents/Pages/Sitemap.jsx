...
const SitemapTable = () => {
  ...
  const fetchProductData = async () => {
    ...
    try {
      ...
      const categoryData = categoryResponse.data.map((item, index) => ({
        id: productData.length + index + 1,  // Ensuring unique ids
        _id: item._id,
        url: item.url,
        priority: item.priority,
        changeFreq: item.changeFreq,
      ...
      // Combine product and category data
  ...
  useEffect(() => {
    setMainSitemaps([...dataSitemaps, ...productSitemaps, ...newsSitemaps]);
  }, [dataSitemaps, productSitemaps, newsSitemaps]);
  ...
  return (
    <div className="p-4 overflow-x-auto">
      ...
      <div className="flex gap-4">
        ...
        <button onClick={generateNewsSitemap} className="bg-slate-700 text-white py-2 px-4 rounded">Generate Blog Sitemap</button>
      </div>
