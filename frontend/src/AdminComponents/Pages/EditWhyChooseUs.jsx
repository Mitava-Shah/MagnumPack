...
const EditWhyChooseUs = () => {
  ...
  const fetchWhyChooseUs = async () => {
    try {
      const response = await axios.get(`/api/WhyChooseUs/getWhyChooseUsById?id=${id}`, { withCredentials: true });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`/api/WhyChooseUs/updateWhyChooseUs?id=${id}`, formData, {
        headers: {
  ...
  const handleDeleteInitialPhoto = (e, photoFilename, index) => {
    e.preventDefault();
    axios.delete(`
/api/whychooseus/${id}/image/${photoFilename}/${index}`, { withCredentials: true })
      .then(() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4">
      <h1 className="text-xl font-bold font-serif text-gray-700 uppercase text-center">Edit Why Choose Us</h1>
      <div className="mb-4">
