...
const CustomInquiries = () => {
  ...
  const saveHeadings = async () => {
    try {
      await axios.put('/api/pageHeading/updateHeading?pageType=custominquiries', {
        pagetype: 'custominquiries',
        heading,
        subheading,
...
/api/custominquiry/getcustomtinquiries`, { withCredentials: true });
...
/api/custominquiry/deleteinquiries?id=${id}`, { withCredentials: true });
      fetchData();
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className="p-4 overflow-x-auto">
      ...
      <h1 className="text-xl font-bold text-gray-700 font-serif uppercase mb-4">Custom Product Inquiries</h1>
      <div className="mb-4">
        <input
          type="text"
                ...
                {rows.map((row) => {
                  ...
                  return (
                    <tr {...row.getRowProps()} className="border-b border-gray-300 hover:bg-gray-100 transition duration-150">
                      {row.cells.map((cell) => (
                        <td {...cell.getCellProps()} className="py-2 px-4 text-sm text-gray-700">{cell.render("Cell")}</td>
                      ))}
                    </tr>
                  );
                })}
