...
/api/news/getall`, { withCredentials: true });
  ...
  const deleteCategory = async ({ id, categoryId, subCategoryId, subSubCategoryId }) => {
    ...
    if (categoryId && subCategoryId && subSubCategoryId) {
      url = `
/api/news/deletesubsubcategory?categoryId=${categoryId}&subCategoryId=${subCategoryId}&subSubCategoryId=${subSubCategoryId}`;
    } else if (categoryId && subCategoryId) {
      url = `
/api/news/deletesubcategory?categoryId=${categoryId}&subCategoryId=${subCategoryId}`;
    } else {
      url = `
/api/news/deletecategory?id=${id}`;
    }
                    ...
                    {row.original.subCategories && row.original.subCategories.map((subcategory, subIndex) => (
                      <React.Fragment key={subIndex}>
                        <tr className="border-b border-gray-300 hover:bg-gray-100 transition duration-150 ">
                          <td></td>
                          <td className="py-2 px-8 flex gap-2 hover:text-blue-500 cursor-pointer" onClick={() => navigate(`/NewsCategory/editNewsCategory/${row.original._id}/${subcategory._id}`)}><BsArrowReturnRight />{subcategory.photo && <img src={`
/api/logo/download/${subcategory.photo}`} alt={subcategory.alt} className="w-6 h-6" />}<span>{subcategory.category}</span></td>
                        ...
                        {subcategory.subSubCategory && subcategory.subSubCategory.map((subSubcategory, subSubIndex) => (
                          <tr key={subSubIndex} className="border-b border-gray-300 hover:bg-gray-100 transition duration-150 ">
                            <td></td>
                            <td className="py-2 px-12 flex gap-2 hover:text-blue-500 cursor-pointer" onClick={() => navigate(`/NewsCategory/editNewsCategory/${row.original._id}/${subSubcategory._id}`)} ><BsArrowReturnRight />{subSubcategory.photo && <img alt={subSubcategory.alt} src={`
/api/logo/download/${subSubcategory.photo}`} className="w-6 h-6" />}<span>{subSubcategory.category}</span></td>
                            <td className="py-2 px-4">
                              <div className="flex gap-4">
                                <button className="text-blue-500 hover:text-blue-700 transition">
