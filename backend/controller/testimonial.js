...
const insertTestimonial = async (req, res) => {
  try {
    const { name, designation, testimony, rating, alt,imgTitle, status } = req.body;
...
const getActiveTestimonials = async (req, res) => {
  try {
    ...
    let errorMessage = 'Error fetching active testimonials';
    if (error.name === 'CastError') {
      errorMessage = 'Invalid query parameter format';
    }
    res.status(500).json({ message: errorMessage });
...
module.exports = { insertTestimonial, getTestimonials,getActiveTestimonials, updateTestimonial, deleteTestimonial, getTestimonialById, countTestimonial, deletePhotoAndAltText };